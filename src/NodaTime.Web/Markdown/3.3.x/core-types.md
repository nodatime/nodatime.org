...
In scenarios where values will always be in the near-past or near-future (e.g. 18th century to 22nd century),
and where performance (either in execution time or memory) is important, the [`Instant64`][Instant64] type may
be appropriate. It has a smaller range and supports a more limited set of operations, but is significantly more efficient.
...
Introduced in Noda Time 2.3, [`OffsetDate`][OffsetDate] and [`OffsetTime`][OffsetTime]
are unlikely to be used very often. They're a `LocalDate` or `LocalTime` respectively,
with an associated offset.

These are far less widely used than `OffsetDateTime`, but may crop up occasionally -
and are officially supported (though rarely used) in XML values, such as `<date>2018-03-25+01:00</date>`
or `<time>10:30:00+01:00</time>`.
...
In scenarios where values will always be "small" (up to about 290 years, either positive or negative)
and where performance (either in execution time or memory) is important, the [`Duration64`][Duration64] type may
be appropriate. It has a smaller range and supports a more limited set of operations, but is significantly more efficient.
...
[Instant64]: noda-type://NodaTime.HighPerformance.Instant64
...
[Duration64]: noda-type://NodaTime.HighPerformance.Duration64
