...
The `Duration64` and `Instant64` types in the `NodaTime.HighPerformance`
namespaces are intended to represent the same concepts are `Duration` and `Instant`,
but with a smaller range of valid values which allows each type to be represented
in 64 bits. (Each is just a wrapper around an `Int64` field, effectively.)

These types are not general-purpose drop-in replacements for `Duration64` and `Instant64`.
Most importantly, they do not support text parsing or formatting, and they aren't designed
to interact with other Noda Time types other than via a conversion to or from `Duration`/`Instant`.

We expect these types to be limited in utility, but where performance is critical (and
conversions to and from the normal Noda Time types can be performed on boundaries) they can
