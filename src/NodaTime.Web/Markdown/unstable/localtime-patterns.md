...
- `r`: Culture-sensitive round-trip pattern.
  This always uses a pattern of "HH:mm:ss.FFFFFFFFF", but with the culture-specific time separator.
- `o`: Invariant round-trip pattern with only required subsecond digits. This always uses a pattern of "HH:mm:ss.FFFFFFFFF" using the invariant culture.
- `O`: Invariant round-trip pattern, always using 9 subsecond digits. This always uses a pattern of "HH:mm:ss.fffffffff" using the invariant culture.
